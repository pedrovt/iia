...
class Cidades(SearchDomain):
    ...
    def cost(self, state, action):
        state_src, state_dest = action
        if (state != state_src):
            return None

        for (src, dest, cost) in self.connections:
            if ((src == state_src and dest == state_dest) or (src == state_dest and dest == state_src)): 
                return cost
    ...
    def heuristic(self, state, goal_state):
        state_x, state_y = self.coordinates[state]
        goal_x, goal_y = self.coordinates[goal_state]
        return hypot(goal_x - state_x, goal_y - state_y)
...
print(t3.search())
print(t4.search())
#print(cidades_portugal.heuristic('Aveiro', 'Porto'))
